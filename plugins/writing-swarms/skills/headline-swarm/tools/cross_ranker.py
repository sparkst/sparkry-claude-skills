...
Note: This is largely the same as soundbite-swarm cross_ranker, adapted for headlines.
"""
...
def aggregate_rankings(rankings: Dict[str, List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
    ...
    # Track scores for tiebreaking (headline-specific)
    candidate_scores: Dict[str, Dict[str, float]] = defaultdict(
        lambda: {"authenticity": 0, "clarity": 0, "curiosity": 0}
    )
    ...
    for angle, ranking_list in rankings.items():
        ...
        for entry in ranking_list:
            ...
            if scores:
                for key in ["authenticity", "clarity", "curiosity"]:
                    if key in scores:
    ...
    # For headlines: authenticity > clarity > angles_ranked > curiosity
    results.sort(
        key=lambda x: (
            x["borda_points"],
            x["tiebreaker_scores"]["authenticity"],
            x["tiebreaker_scores"]["clarity"],
            x["angles_ranked"],
            x["tiebreaker_scores"]["curiosity"]
        ),
        reverse=True
    )
