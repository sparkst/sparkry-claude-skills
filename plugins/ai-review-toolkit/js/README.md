...
- **In-place fixer, no worktree isolation.** Fixes persist so the next round's
  reviewers see them.
- Loop semantics (min-2-rounds floor, reviewer attestation + the quorum gate,
  fix-ALL gate, stuck detection, max-rounds escalation) live in `loop-engine.mjs`
  itself now -- the Python `loop-driver.py`
