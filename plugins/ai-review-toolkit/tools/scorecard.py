...
def _verdict_section(
    ...
    # #52: the loop reports the sentinel "n/a" when the runtime has no clock
    # (Workflow scripts forbid Date.now). That sentinel outranks any wall-clock
    # the transcript infers, so the verdict never prints an invented duration.
    budget_wall = budget.get("wallClockMs")
    if budget_wall == "n/a":
        wall = "n/a"
    else:
        wall = time_section.get("workflow_wall_clock_ms")
        if wall is None:
            wall = budget_wall or time_section.get("total_ms")
