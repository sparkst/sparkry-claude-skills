...
- `runLoop` returns `budget` (`maxRounds`, `roundsRun`, `validRounds`, `invalidRounds`, `fullRounds`, `deltaRounds`, `gateRounds`, `reviewedRounds`, `maxInvalidRounds`, `hardCap`, `wallClockMs`); `history` entries carry `kind`, `ms`, `delta_reason`, reviewer counts. `wallClockMs` and per-round `ms` are `null` (never a fabricated number) when the runtime has no clock (Workflow scripts forbid `Date.now`); `scorecard.py` falls back to the harness-measured wall-clock when the loop's own is null.
