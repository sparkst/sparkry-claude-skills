# /qloop — Legacy Python-Driver Protocol (fallback)

> **Fallback only.** As of v0.2.0, `/qloop` runs via the ultracode Workflow
> (`review-loop.workflow.js`, until-converged) — see `SKILL.md`. This hand-driven
> Python-driver protocol is retained for one release as a fallback for when the
> Workflow tool is unavailable, and will be removed when the drivers are deleted
> (step 7 of the ultracode refactor).

# /qloop -- Iterative Review-Fix-Verify Loop (legacy)
